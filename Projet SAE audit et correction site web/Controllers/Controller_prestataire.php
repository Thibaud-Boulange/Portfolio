...
class Controller_prestataire extends Controller
{
    ...
    public function action_afficher_bdl()
    {
        ...
        if (isset($_GET['id'])) {
            ...
            $data = ['menu' => $this->action_get_navbar(), 'bdl' => $typeBdl];
    ...
    {
        $bd = Model::getModel();
        $data = ['menu' => $this->action_get_navbar(), 'composantes' => $bd->getDashboardPrestataire($_SESSION['id'])] ;
    ...
    {
        if(maj_infos_personne()){
            $this->action_error("Il y a eu un soucis lors de la modification du mot de passe !"); 
        } // fonction dans Utils
        $this->action_infos();
    }
