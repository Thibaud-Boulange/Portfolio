...
class Controller_administrateur extends Controller
{
    ...
    public function action_get_navbar()
    {
        return [['link' => '?controller=administrateur&action=clients', 'name' => 'Société'],
            ...
            ['link' => '?controller=administrateur&action=getPersonnes', 'name' => 'Utilisateurs']
    ...
    public function action_ajout_interlocuteur_form()
    {
        ...
        if(isset($_GET['id-client'])) {
            $composante = $bd->getNomsComposanteClient($_GET['id-client']);
    ...
    public function action_getPersonnes(){
        $bd = Model::getModel();
      $res = $bd -> getAllPersonnes();
      $data = ['personnes'=>$res,'menu' => $this->action_get_navbar()];
        
        $this->render("gestionPersonnes",$data);
    }
  
  
    //fonction permettant de gerer les requetes ajax depuis la page d'administration des utilisateurs
    public function action_getInfosPersonne(){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $bd = Model::getModel();

            $boutonId = isset($_POST['boutonId']) ? $_POST['boutonId'] : null;
            if ($boutonId !== null) {
                $res = $bd -> getInfosPersonneByEmail($boutonId);
                $response = [
                    'status' => 'success',
                    'id' => $res["id_personne"],
                    'nom' => $res["nom"],
    ...
    public function action_delPersonne() {
        if (isset($_GET['role']) && isset($_GET['id'])) {
            ...
            try {
                $bd->deleteRole($role, $id);
                // Rediriger vers la page des utilisateurs après la suppression
                header('Location: ?controller=administrateur&action=getPersonnes');
                exit();
            } catch (Exception $e) {
                // Gérer l'erreur et rediriger en cas d'échec
                header('Location: ?controller=administrateur&action=getPersonnes&error=' . urlencode('Erreur lors de la suppression du rôle : ' . $e->getMessage()));
                
                exit();
            }
        } else {
            // Rediriger en cas de paramètres manquants
            header('Location: ?controller=administrateur&action=getPersonnes&error=' . urlencode('Paramètres manquants'));
            exit();
        }
    }
    
    public function action_ajouterRolePersonne()
    {
        if (isset($_GET['role']) && isset($_GET['id'])) {
            $role = $_GET['role'];
            $id = $_GET['id'];
            ...
            var_dump($id);
            try {
                $bd->ajouterRolePersonne($role, $id);
                // Rediriger vers la page des utilisateurs après l'ajout du rôle
                header('Location: ?controller=administrateur&action=getPersonnes');
                exit();
            } catch (Exception $e) {
                // Gérer l'erreur et rediriger en cas d'échec
                header('Location: ?controller=administrateur&action=getPersonnes&error=' . urlencode('Erreur lors de l\'ajout du rôle : ' . $e->getMessage()));
                exit();
            }
        } else {
            // Rediriger en cas de paramètres manquants
