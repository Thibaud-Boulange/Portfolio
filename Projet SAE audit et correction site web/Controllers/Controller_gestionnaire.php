...
class Controller_gestionnaire extends Controller
{
    ...
    {
        $bd = Model::getModel();
        $data = ['menu' => $this->action_get_navbar(), 'ls_commerciaux' => $bd->getAllCommerciaux(), 'cp' => $bd->getAllLocalite(), 'typeVoie' => $bd->getAllTypeVoie(), 'clients' => $bd->getAllClients()];
    ...
    {   
        ...
        $ajout_possible = [];
        if(isset($_GET['id'])){
            $liste = $bd->getAllCommerciaux();
            $existant = $bd->getCommerciauxComposante($_GET['id']);
            foreach($liste as $p){
                if(!in_array($p, $existant)) {
                    $ajout_possible[] = ['name' => $p['nom'].' '.$p['prenom'], 'email' => $p['email']];
                }
            }
        }
        else {
            $liste = $bd->getAllPersonnes();
            foreach($liste as $p){
                if(count($p['role']) == 0 or (in_array('gestionnaire', $p['role']) and !in_array('commercial', $p['role']))){
                    $ajout_possible[] = ['name' => $p['nom'].' '.$p['prenom'], 'email' => $p['email']];
                }
            }
        }
        $data = ['menu' => $this->action_get_navbar(), 'liste' => $ajout_possible];
    ...
    public function action_ajout_composante()
    {
        ...
        if (isset($_POST['composante']) &&
            ...
            $this->action_error('Il manque des informations ou la composante existe déjà !'); 
        }
        if(isset($_POST['tel'])){
            $this->action_ajout_client_form();
        }
    ...
    {
        $bd = Model::getModel();
        if (isset($_POST['email-interlocuteur']) && isset($_POST['nom-interlocuteur']) && isset($_POST['prenom-interlocuteur'])){
            ...
            if (isset($_GET['id-composante']) or (isset($_POST['client']) && isset($_POST['composante']))){
                if (isset($_GET['id-composante'])){
                    $id['id_composante'] = $_GET['id-composante'];
                }
                else {
                    $id = $bd->getIdComposante(e($_POST['composante']), e($_POST['client']));
                }
                $tabcomposante = $bd->getInterlocuteursComposante($id['id_composante']);
                foreach($tabcomposante as $cle => $value) { 
                    if($value['email'] == $_POST['email-interlocuteur']){
                        $this->action_error('Cet interlocuteur a déjà été ajouté à cette composante !'); 
                    }
                }
            ...
            else if (isset($_GET['id-client'])) {
                $idComposante = $bd->getIdComposante($_POST['composante'], intval($_GET['id-client'])); 
                $tabcomposante = $bd->getInterlocuteursComposante($idComposante['id_composante']);
                foreach($tabcomposante as $cle => $value) { 
                    if($value['email'] == $_POST['email-interlocuteur']){
                        $this->action_error('Cet interlocuteur a déjà été ajouté à cette composante !'); 
                    }
