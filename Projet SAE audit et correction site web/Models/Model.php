...
class Model
{
    ...
    {
        $req = $this->bd->prepare('SELECT personne.id_personne AS id, nom, prenom, email FROM PERSONNE JOIN COMMERCIAL USING(id_personne)');
        $req->execute();
        return $req->fetchall(PDO::FETCH_ASSOC);
    }
    ...
    /**
     * Méthode permettant de récupérer la liste de toutes les ssociétés
     * @return array|false
     */
    ...
    public function getAllPersonnes() {
        $req = $this->bd->prepare('SELECT id_personne, nom, prenom, email FROM PERSONNE');
        $req->execute();
        $liste = $req->fetchAll(PDO::FETCH_ASSOC); // Utilisation de PDO::FETCH_ASSOC pour des clés nommées
    
        foreach ($liste as &$row) { // Utilisation de & pour obtenir une référence à chaque élément
            $id = $row['id_personne'];
    ...
    public function getInfosPersonneByEmail($mail){
        $req = $this->bd->prepare('SELECT id_personne, nom, prenom, email FROM PERSONNE WHERE email = :mail');
        $req->bindValue(':mail', $mail, PDO::PARAM_STR);
        $req->execute();
        $liste = $req->fetchall()[0];
        $liste["roles"]=$this->getRole($liste["id_personne"]);
        return $liste;
    }


    public function deleteRole($role,$id){
        $sql="DELETE FROM $role WHERE id_personne = :id";
        $req = $this->bd->prepare($sql);
        $req->bindValue(':id', $id, PDO::PARAM_INT);
        $req->execute();
    }

    public function ajouterRolePersonne($role, $id){
        $sql= "INSERT INTO $role (id_personne) VALUES ($id)";
        $req = $this->bd->prepare($sql);
        $req->execute();
    }
    ...
    public function getAllLocalite() {
        $req = $this->bd->prepare('SELECT * FROM localite');
        $req->execute();
        return $req->fetchall(PDO::FETCH_ASSOC);
    }

    public function getAllTypeVoie() {
    ...
    public function getIdComposante($composante, $client)
    {
        if (gettype($client) == 'integer'){
            $req = $this->bd->prepare('SELECT id_composante FROM COMPOSANTE WHERE nom_composante = :composante and id_client = :client ');
        }
        else {
    ...
    public function getNomsComposanteClient($id)
    {
        $req = $this->bd->prepare('SELECT nom_composante FROM composante WHERE id_client = :id');
        $req->bindValue(':id', $id, PDO::PARAM_INT);
        $req->execute();
        return $req->fetchall(PDO::FETCH_ASSOC);
    }
    ...
    {
        $req = $this->bd->prepare('SELECT DISTINCT id_personne AS id, nom, prenom, email
    ...
    {
        $req = $this->bd->prepare('SELECT DISTINCT id_personne, nom, prenom, email
    ...
    //FONCTION DE RECUPERANT LE ROLE D'UN USER
    public function getRole($id){
        $roles = [];
        $gestionnaire = $this->isGestionnaire($id);
        $admin = $this-> isAdmin($id);
        $prestataire =  $this-> isPrestataire($id);
        $commercial = $this-> isCommercial($id);
        $interlocuteur = $this-> isInterlocuteur($id);
        ...
        if($interlocuteur==TRUE){
            $roles[] = "interlocuteur";
        }
        return $roles;

    }
    //FONCTIONS VERIFIANT LE STATUS DU ROLE DE LA PERSONNE EN PARAMETRE
   
    public function isAdmin($id){
       $query=$this->bd->prepare(" SELECT EXISTS (
            SELECT *
            FROM administrateur
    ...
    public function isCommercial($id){
        $query=$this->bd->prepare(" SELECT EXISTS (
            SELECT *
            FROM commercial
            WHERE id_personne = $id 
    ...
    public function isGestionnaire($id){
        $query=$this->bd->prepare(" SELECT EXISTS (
            SELECT *
            FROM gestionnaire
            WHERE id_personne = $id 
    ...
    public function isInterlocuteur($id){
        $query=$this->bd->prepare(" SELECT EXISTS (
            SELECT *
            FROM interlocuteur
            WHERE id_personne = $id 
    ...
    public function isPrestataire ($id){
        $query=$this->bd->prepare(" SELECT EXISTS (
            SELECT *
            FROM prestataire
            WHERE id_personne = $id 
    ...
    {
        $req = $this->bd->prepare('SELECT nom_mission, date_debut, nom, prenom, email, id_bdl, ta.id_mission, ta.id_personne as id_prestataire FROM mission m JOIN travailleAvec ta USING(id_mission) JOIN personne p USING(id_personne) JOIN bon_de_livraison bdl ON m.id_mission= bdl.id_mission WHERE bdl.id_interlocuteur = :id;');
    ...
    {
        $req = $this->bd->prepare('SELECT email from personne;');
        $req->execute();
        return $req->fetchAll(PDO::FETCH_ASSOC);
    }
